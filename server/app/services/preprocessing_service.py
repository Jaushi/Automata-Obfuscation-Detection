...
def word_tokenize(clean_text: str) -> list[str]:
    ...
    tokens = []
    # Match words (alphanumeric + some special chars including asterisks for censored words) and punctuation separately
    pattern = r'([a-z0-9@!$|*]+)|([^\s])'
...
def extract_word(token: str) -> str:
    """Extract word part from token, including leetspeak chars and asterisks, removing only punctuation."""
